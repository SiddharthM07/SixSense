...
def predict(match_id):
    ...
    batsmen = [
        ...
        if "Batsman" in p["role"]
        or "WK-Batsman" in p["role"]
        or "Batting Allrounder" in p["role"]
    ]
    bowlers = [
        p
        for p in team1_players + team2_players
    ...
    return render_template(
        "predict.html", match=match, batsmen=batsmen, bowlers=bowlers
    )
...
def submit_prediction():
    try:
        ...
        match_time_ist = datetime.strptime(
            match_time_str, "%Y-%m-%dT%H:%M:%S"
        )  # Parse correctly
        ...
        if match_time_utc - current_time_utc < timedelta(
            minutes=10
        ):  # ✅ Now timedelta works
...
def view_results(match_id):
    ...
    results_response = (
        supabase.table("match_results").select("*").eq("match_id", match_id).execute()
    )
    ...
    user_prediction = (
        user_prediction_response.data[0] if user_prediction_response.data else None
    )
    ...
    if user_prediction:
        ...
        match_score = (
            match_score_response.data[0]["score"] if match_score_response.data else 0
        )
    else:
    ...
    total_score_response = (
        supabase.table("user_scores").select("score").eq("user_id", user_id).execute()
    )
    total_score = (
        sum(entry["score"] for entry in total_score_response.data)
        if total_score_response.data
        else 0
    )
