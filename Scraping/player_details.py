...
def get_completed_matches():
    response = supabase.table("matches").select("match_id").eq("status", "completed").execute()
...
def extract_match_results(match_data):
    ...
    print(f"Top Wicket Taker: {top_wicket_taker} ({max_wickets} wickets)")  # Debugging output
...
def update_match_results():
    ...
    for match_id in match_ids:
        ...
        print(f"Updated results for match {match_id}: Winner - {winner}, Top Batsman - {top_batsman}, Top Wicket Taker - {top_wicket_taker}")
